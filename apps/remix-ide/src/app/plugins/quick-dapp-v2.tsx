...
export const profile = {
  ...
  permission: true,
  events: [],
  methods: ['edit', 'clearInstance', 'startAiLoading', 'createDapp', 'createDappWorkspace', 'createZkDapp', 'createZkDappWorkspace', 'createNoirZkDappWorkspace', 'openDapp', 'consumePendingCreateDapp', 'listDapps', 'prepareFigmaDesign', 'getPreparedFigmaDesign', 'clearPreparedFigmaDesign']
}
...
export class QuickDappV2 extends ViewPlugin {
  ...
  async createZkDappWorkspace(payload: {
    ...
    try {
      const verifierContractPath = `${extractParentFromKey(payload.circuitPath)}/${payload.provingScheme}/zk/build/zk_verifier.sol`;
      verifierContractContent = await this.call('fileManager', 'readFile', verifierContractPath);
    } catch (e) {
    ...
    const verificationMethod = payload.verificationMethod || 'zkverify';
    const initialConfig = {
      ...
      zkCircuit: {
        ...
        verificationMethod,
        onChainVerifier: verificationMethod === 'onchain' ? payload.onChainVerifier : undefined
      },
      sourceWorkspace: {
      ...
      config: {
        title: name,
        details: payload.userDescription || `ZK DApp with in-browser proof generation and ${verificationMethod === 'onchain' ? 'on-chain contract' : 'zkVerify'} verification`
      }
    };
