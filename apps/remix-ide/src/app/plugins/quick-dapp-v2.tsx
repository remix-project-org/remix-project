...
export class QuickDappV2 extends ViewPlugin {
  ...
  async createDappWorkspace(payload: {
    ...
    const legacyPrimary = {
      id: 'legacy-primary',
      alias: payload.contractName || 'Contract',
    ...
    const contractSelection = hasExplicitContracts
      ...
      : {
        contracts: [legacyPrimary],
        primaryContractId: legacyPrimary.id,
    ...
    const id = Date.now().toString(36) + Math.random().toString(36).substring(2, 8);
    const slugSuffix = hasExplicitContracts ? id.slice(-6) : id.slice(0, 6);
    ...
    if (hasExplicitContracts && await this.call('filePanel' as any, 'workspaceExists', workspaceName)) {
    ...
    if (hasExplicitContracts && vmProviderName) {
    ...
    if (vmProviderName && hasExplicitContracts) {
      try {
    ...
    if (hasExplicitContracts && vmProviderName && !vmStateSnapshot) {
      throw new Error('createDappWorkspace: Could not capture Remix VM state for all selected contracts');
    }

    const contractConfigFields = hasExplicitContracts
    ...
    const rollbackMultiContractCreation = async () => {
      if (!hasExplicitContracts) return;
    ...
    for (const contract of contractSelection.contracts) {
      try {
        ...
        const preserveExistingPin = hasExplicitContracts && await this.call('fileManager', 'exists', pinPath);
        if (!preserveExistingPin) {
          await this.call('fileManager', 'writeFile', pinPath, JSON.stringify(pinnedData, null, 2));
          if (hasExplicitContracts) createdSourcePinPaths.push(pinPath);
        }
        ...
        if (contract.id === contractSelection.primaryContractId) {
          ...
          const preserveExistingMapping = hasExplicitContracts && await this.call('fileManager', 'exists', sourceMappingPath);
          if (!preserveExistingMapping) {
            await this.call('fileManager', 'writeFile', sourceMappingPath, JSON.stringify(dappMapping, null, 2));
            if (hasExplicitContracts) sourceMappingCreated = true;
          }
        }
      } catch (e) {
        if (hasExplicitContracts) {
