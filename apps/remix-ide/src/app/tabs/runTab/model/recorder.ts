...
export class Recorder extends Plugin {
  ...
  /**
    ...
    *
    */
  async run (records, accounts, options, abis, linkReferences, liveMode) {
    ...
    try {
      for (let index = 0; index < records.length; index++) {
        const tx: JournalEntry = records[index]
        ...
        if (!abi) {
          ...
          throw new Error('cannot find ABI for ' + tx.record.abi + '.  Execution stopped at ' + index)
        }
        /* Resolve Library */
        if (record.linkReferences && Object.keys(record.linkReferences).length) {
        ...
        if (!fnABI) {
          ...
          throw new Error('cannot resolve abi of ' + JSON.stringify(record, null, '\t') + '. Execution stopped at ' + index)
        }
        if (tx.record.parameters) {
          /* check if we have some params to resolve */
          try {
            tx.record.parameters.forEach((value, paramIndex) => {
              let isString = true
            ...
            throw new Error('cannot resolve input parameters ' + JSON.stringify(tx.record.parameters) + '. Execution stopped at ' + index)
          }
        }
        ...
        if (data.error) {
          ...
          throw new Error(data.error + '. Record:' + JSON.stringify(record, null, '\t') + '. Execution stopped at ' + index)
        }
        ...
        try {
          const { address: rawAddress } = await this.blockchain.runTx(record)
          const address = addressToString(rawAddress)

          // save back created addresses for the conversion from tokens to real addresses
          this.data._createdContracts[address] = tx.timestamp
          this.data._createdContractsReverse[tx.timestamp] = address
          return { abi, address, contractName: record.contractName }
        } catch (err) {
          console.error(err)
          // return logCallBack(err + '. Execution failed at ' + index)
          throw new Error(err + '. Execution failed at ' + index)
        }
      }
    } finally {
      this.setListen(true)
  ...
  runScenario (liveMode, json) {
    ...
    return this.run(txArray, accounts, options, abis, linkReferences, liveMode)
  }
}
