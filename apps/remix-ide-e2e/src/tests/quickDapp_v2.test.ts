...
module.exports = {
  ...
  // ──────────────────────────────────
  // Test 1: Enable experimental mode, compile+deploy contract, and trigger QuickDapp V2 via sparkle button
  // ──────────────────────────────────
  'Should enable experimental mode and compile and deploy Storage contract #group1': function (browser: NightwatchBrowser) {
    browser
  ...
  // ──────────────────────────────────
  // Test 2: Click sparkle button, verify AI modal, enter description, and generate
  // ──────────────────────────────────
  ...
  'Should create a second DApp and delete from dashboard #group1': function (browser: NightwatchBrowser) {
    browser
      ...
      .pause(3000)
      // Should show Getting Started screen
      .waitForElementVisible('*[data-id="quickdapp-getting-started"]', 30000)
      .assert.textContains('*[data-id="quickdapp-getting-started"]', 'Getting Started')
      .end()
  },
