...
export function NotificationBell({ className = '' }: NotificationBellProps) {
  ...
  useEffect(() => {
    ...
    // Initial fetch
    plugin
      .call('notificationCenter', 'getUnreadCount' as any)
      .then((count: number) => {
        setUnreadCount(count)
      })
      .catch(() => {})
  ...
  const handleLegacyActionUrl = useCallback(
    (actionUrl: string) => {
      setIsOpen(false)
  ...
  return (
    <div data-id="notification-bell-container" className={`notification-bell-container ${className}`} ref={dropdownRef}>
      <CustomTooltip placement="bottom" tooltipText="Notifications">
        <span className="notification-bell-icon" onClick={toggleDropdown} data-id="notification-bell">
      ...
      {isOpen && (
        <div className="notification-dropdown" data-id="notification-dropdown">
          <div className="notification-dropdown-header">
            ...
            {unreadCount > 0 && (
              <button className="notification-mark-all-btn" onClick={handleMarkAllAsRead} title="Mark all as read">
                ...
                >
                  ...
                  <div className="notification-item-content">
                    <div className="notification-item-header">
                      {notification.read_status === null && <div className="notification-unread-dot"></div>}
                      <span className={`notification-item-title ${notification.priority === 'high' || notification.priority === 'critical' ? 'fw-bold' : ''}`}>{notification.title}</span>
                  ...
                  <button className="notification-dismiss-btn" onClick={(e) => handleDismiss(notification.id, e)} title="Dismiss">
