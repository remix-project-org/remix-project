...
export class DappManager {
  ...
  async createZkDapp(payload: CreateZkDappPayload): Promise<DappConfig> {
    const { circuitName, circuitPath, provingScheme, primeValue, signalInputs, wasmPath, zkeyPath, verificationKey, zkVerifyNetwork, userDescription } = payload;
    ...
    const initialConfig: DappConfig = {
      ...
      config: {
        title: circuitName,
        details: userDescription || 'ZK DApp with in-browser proof generation and zkVerify verification'
      }
    };
