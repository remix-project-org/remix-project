...
import { extractParentFromKey, logQuickDappBinding, readQuickDappContractConfig } from '@remix-ui/helper';
...
export class DappManager {
  ...
  async getDapps(): Promise<DappConfig[]> {
    try {
      ...
      const bindingSources = uniqueConfigs.reduce((counts, config) => {
        const { source } = readQuickDappContractConfig(config);
        counts[source]++;
        return counts;
  ...
  async createZkDapp(payload: CreateZkDappPayload): Promise<DappConfig> {
    const { circuitName, circuitPath, provingScheme, primeValue, signalInputs, wasmPath, zkeyPath, verificationKey, zkVerifyNetwork, userDescription, verificationMethod, onChainVerifier } = payload;
    ...
    try {
      ...
      const wasmData = await (this.plugin as any).call('fileManager', 'readFile', wasmPath, { encoding: null });
      wasmContent = wasmData instanceof Uint8Array ? wasmData : new TextEncoder().encode(wasmData as string);
    } catch (e: any) {
      console.warn('[DappManager] Failed to read wasm file:', e);
    }

    try {
      const zkeyData = await (this.plugin as any).call('fileManager', 'readFile', zkeyPath, { encoding: null });
      zkeyContent = zkeyData instanceof Uint8Array ? zkeyData : new TextEncoder().encode(zkeyData as string);
    } catch (e) {
      console.warn('[DappManager] Failed to read zkey file:', e);
    }

    let verifierContractContent: string | null = null;
    try {
      const verifierContractPath = `${extractParentFromKey(circuitPath)}/${provingScheme}/zk/build/zk_verifier.sol`;
      verifierContractContent = await (this.plugin as any).call('fileManager', 'readFile', verifierContractPath);
    } catch (e) {
      // Not generated (checkbox unchecked) - skip silently.
    }
    ...
    // Write circuit artifacts
    if (wasmContent) {
      try {
        await (this.plugin as any).call('fileManager', 'writeFile', 'zk/circuit.wasm', wasmContent, { silent: true });
      } catch (e) {
        console.warn('[DappManager] Failed to write wasm file:', e);
      }
    }

    if (zkeyContent) {
      try {
        await (this.plugin as any).call('fileManager', 'writeFile', 'zk/circuit.zkey', zkeyContent, { silent: true });
      } catch (e) {
        console.warn('[DappManager] Failed to write zkey file:', e);
      }
    }

    // Write verification key as JSON
    try {
      await (this.plugin as any).call(
        'fileManager', 'writeFile',
      ...
      console.warn('[DappManager] Failed to write verification key:', e);
    }

    if (verifierContractContent) {
      try {
    ...
    const initialConfig: DappConfig = {
      ...
      zkCircuit: {
        ...
        zkVerifyConfig: zkVerifyNetwork ? { network: zkVerifyNetwork } : undefined,
        verificationMethod: verificationMethod || 'zkverify',
        onChainVerifier: verificationMethod === 'onchain' ? onChainVerifier : undefined
      },
      sourceWorkspace: {
        name: sourceWorkspaceName,
