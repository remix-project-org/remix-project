...
export function TopCard(props: TopCardProps) {
  ...
  return (
    <div
      className={`explora-topcard flex flex-row items-center bg-light p-4 shadow-sm ${props.importWorkspace ? 'border bg-transparent' : 'border-0'}`}
    ...
    >
      ...
      <span className="flex flex-col flex-grow-1 ml-3">
        <p className="mb-0">{props.title}</p>
        <p className="mb-0 font-light whitespace-normal">{props.description}</p>
      </span>
    </div>
  )
}
