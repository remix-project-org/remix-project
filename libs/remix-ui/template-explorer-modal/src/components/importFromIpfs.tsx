...
export function ImportFromIpfs(props: any) {
  ...
  return (
    <section className="flex flex-col gap-3 bg-light" style={{ height: '80%' }}>
      <div className="pt-3 flex flex-col text-dark mx-3 my-3">
        ...
        {externalResourceNameError.length > 0 && externalResourceName.length > 0 ? <span className="text-danger font-light mt-1 justify-start fs-6">{externalResourceNameError}</span> : null }
      </div>

      <button className="inline-flex items-center px-4 py-2 bg-primary text-white rounded-md hover:bg-primary/90 transition-colors btn-lg mx-3" data-id="validateWorkspaceButton" onClick={async () => {
