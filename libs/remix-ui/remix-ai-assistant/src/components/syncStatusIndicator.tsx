...
export const SyncStatusIndicator: React.FC<SyncStatusIndicatorProps> = ({ status, lastSyncTime, onManualSync }) => {
  const getStatusIcon = () => {
    switch (status) {
      case 'syncing':
  ...
  return (
    <CustomTooltip tooltipText={getStatusText()}>
      <button className={`btn btn-sm btn-link sync-status-indicator ${getStatusClass()}`} onClick={handleClick} disabled={status === 'syncing'} data-id="sync-status-indicator sync-manual-trigger" data-sync-status={status}>
        {getStatusIcon()}
      </button>
    </CustomTooltip>
  )
