...
export function SmartAccountPrompt({ plugin }: { plugin: EnvironmentPlugin }) {
  ...
  return (
    <div className="w-100" data-id="createSmartAccountModal">
      ...
      <a href="https://docs.safe.global/advanced/smart-account-overview#safe-smart-account" target="_blank" rel="noreferrer noopener" onClick={() => trackMatomoEvent({ category: 'udapp', action: 'smartAccountLearnMore', name: 'clicked', isClick: true })} className="mb-3 d-inline-block link-primary">
      ...
      <CustomTooltip placement="top" tooltipClasses="text-wrap" tooltipId="createSmartAccountOwnerTooltip" tooltipText={intl.formatMessage({ id: 'udapp.ownerAddressForSmartAccount' })}>
        <input type="text" className="form-control" value={displayAccount} disabled readOnly />
      </CustomTooltip>
    </div>
  )
}
