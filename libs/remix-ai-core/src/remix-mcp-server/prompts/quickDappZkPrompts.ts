...
export interface QuickDappZkOnChainVerifier {
  address: string
  abi: any[]
...
export interface QuickDappZkPromptContext {
  ...
  onChainVerifier?: QuickDappZkOnChainVerifier
}
...
export const buildCreateZkDappPrompt = (args: {
  ...
  const verificationMethod = zkContext.verificationMethod || 'zkverify'
  const isOnChain = verificationMethod === 'onchain'
  ...
  const setupQuestions = isOnChain
    ? [
      ...
      '',
      'Ask exactly those three options only. Do not ask about wallet connection - this DApp verifies proofs on-chain, so wallet connect is required and already enabled. Do not ask for circuit details, ZK_CONTEXT_JSON, Theme, Primary Color, DApp Title, or any other questions.',
  ...
  const generateArgs = [
    ...
    ...(isOnChain ? ['- onChainVerifier: [use the full JSON object from ZK_CONTEXT_JSON below]'] : []),
  ...
  return [
    `I want to create a QuickDapp for a ZK circuit with in-browser proof generation and ${isOnChain ? 'on-chain verifier contract' : 'zkVerify'} verification.`,
    '',
    ...
    ...(isOnChain ? [`On-Chain Verifier: [FULL JSON OBJECT PROVIDED IN ZK_CONTEXT_JSON BELOW]`] : []),
    ...
    ...generateArgs,
    '',
    'ZK_CONTEXT_JSON (contains verificationKey and onChainVerifier - use this for generate_zk_dapp):',
    '```json',
