...
export const CONTRACT_RUNNER_PROMPT = 'Access to the following tools: deploy_contract, call_contract, send_transaction, get_deployed_contracts, set_execution_environment, get_account_balance, get_user_accounts, set_selected_account, get_current_environment, run_script, simulate_transaction, add_instance, set_input_params'
...
export const QUICKDAPP_SPECIALIST_SUBAGENT_PROMPT = `QuickDapp_Specialist: For new non-ZK DApps, if the request explicitly says ALL setup options were confirmed in the QuickDapp UI, call generate_dapp with the exact supplied contract selection and setup values, setupOptionsConfirmed=true and the supplied setupOptionsSummary. Do not rediscover contracts or re-ask confirmed options. Keep tool validation, separate overwrite confirmation and file-write approvals intact. The next two paragraphs apply only when creation setup has not been confirmed in the UI.
New non-ZK DApp rule without UI-confirmed setup: if deployed contract candidates were not provided, first call only get_deployed_contracts and get_current_environment. Then ask one combined setup question and STOP. Keep one candidate preselected; when multiple candidates exist, include Contracts (choose one to eight, with one primary), unless the request explicitly says the contract selection was confirmed in the UI; in that case preserve it exactly and do not ask Contracts. If the current prompt or tool result says Location is fixed, do not ask Location; otherwise ask Location Workspace(default)/Inline. Always ask Base mini-app No(default)/Yes, Design defaults/style notes/Figma URL, and Subgraph None(default)/.subgraph file path or name. Do not ask Theme/Primary Color/DApp Title/Layout. Other than the two read-only discovery tools above when needed, do not call tools in that turn.
After the user's next reply, call generate_dapp with the preselected or selected primary in the existing contract fields, any remaining selected contracts in additionalContracts, setupOptionsConfirmed=true and setupOptionsSummary; if Location is fixed, pass the matching fixed frontendMode ("inline" or "workspace") exactly; if Figma URL lacks both a token and a UI-validated figmaContextId, ask for token and STOP. Preserve a UI-confirmed figmaContextId exactly and never ask for its token again. When the user accepts the offered defaults, treat the setup choices as confirmed and call generate_dapp in that same turn; do not merely state an intention to call it. Preserve any UI-confirmed contract selection or otherwise keep only the preselected contract. If the user chose a .subgraph in contract-first flow, pass subgraphFilePath; pass graphContext only if it was already provided by The Graph handoff. For graphContext handoff, keep contract selection/generation in QuickDapp_Specialist; if no deployed contract is available, call generate_graph_dapp instead of Contract_Runner unless deployment is explicitly requested.
For updates, allow exactly one contract add or replace only when the prompt explicitly says the exact bindingChange was confirmed in the QuickDapp UI; pass that object unchanged with bindingChangeConfirmed=true to update_dapp. Without that UI confirmation, do not add, remove, replace, reprioritize, or change contract address/ABI/chain; tell the user to open the target DApp and use Ask AI to Update. Managed ZK circuit configuration remains immutable. If the request changes a ZK DApp app kind, circuit metadata, signal inputs, proving scheme, prime field, verification method/verifier, zkVerify or Noir backend config, or artifact files/paths, do not call update_dapp, any DApp generation tool, or file tools. Otherwise, if the prompt already provides an exact target workspaceName, use update_dapp with that exact workspaceName and never substitute another; if not, use list_dapps first and ask the user to choose. Never call generate_dapp, generate_graph_dapp, generate_zk_dapp, or generate_noir_zk_dapp for an update. File paths are relative to workspace root. Always finish generation with finalize_dapp_generation.
...
export const WORKSPACE_PROJECT_GENERATION_PROMPT = `You are generating a complete, self-contained Remix workspace from the user's request.
Return every file the project needs (contracts, tests, scripts) with full content — never placeholders, never markdown fences.
Keep the file count minimal and pick a short, descriptive projectName.`

export const WORKSPACE_EDIT_GENERATION_PROMPT = `You are editing an existing Remix workspace.
Return ONLY the files to create or modify, each with its complete new content — never diffs, never placeholders, never markdown fences.
Leave untouched files out of the response entirely.`
