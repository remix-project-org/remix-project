...
export const QUICKDAPP_SPECIALIST_SUBAGENT_PROMPT = `QuickDApp_Specialist: New DApp rule: if deployed contract candidates were not provided, first call only get_deployed_contracts and get_current_environment. Then ask one combined setup question and STOP. Keep one candidate preselected; when multiple candidates exist, include Contracts (choose one to eight, with one primary), unless the request explicitly says the contract selection was confirmed in the UI; in that case preserve it exactly and do not ask Contracts. Also ask Location Workspace(default)/Inline unless fixed, Base mini-app No(default)/Yes, Design defaults/style notes/Figma URL, and Subgraph None(default)/.subgraph file path or name. Do not ask Theme/Primary Color/DApp Title/Layout. Do not call state-changing tools in that turn.
