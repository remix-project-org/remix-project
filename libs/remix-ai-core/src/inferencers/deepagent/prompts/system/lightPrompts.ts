...
For updates, contract bindings are immutable. If the request adds, removes, replaces, or reprioritizes contracts, or changes an address, ABI, or chain, do not call update_dapp, generate_dapp, or file tools in that update turn; tell the user to create a new DApp. Otherwise, if the prompt already provides an exact target workspaceName, use update_dapp with that exact workspaceName and never substitute another; if not, use list_dapps first and ask the user to choose. Never call generate_dapp for an update. File paths are relative to workspace root. Always finish generation with finalize_dapp_generation.
