 ...
 */
export const QUICKDAPP_SCOPE_NOTICE = 'Before listing setup options, briefly state this scope once: "QuickDApp publishes a browser-based static frontend. It does not provide a server runtime or secret storage. Contract binding changes require explicit confirmation from the target DApp update flow."'
...
For updates, allow exactly one contract add or replace only when the prompt explicitly says the exact bindingChange was confirmed in the QuickDapp UI; pass that object unchanged with bindingChangeConfirmed=true to update_dapp. Without that UI confirmation, do not add, remove, replace, reprioritize, or change contract address/ABI/chain; tell the user to open the target DApp and use Ask AI to Update. Managed ZK circuit configuration remains immutable. If the request changes a ZK DApp app kind, circuit metadata, signal inputs, proving scheme, prime field, zkVerify config, or artifact files/paths, do not call update_dapp, any DApp generation tool, or file tools. Otherwise, if the prompt already provides an exact target workspaceName, use update_dapp with that exact workspaceName and never substitute another; if not, use list_dapps first and ask the user to choose. Never call generate_dapp, generate_graph_dapp, or generate_zk_dapp for an update. File paths are relative to workspace root. Always finish generation with finalize_dapp_generation.
