...
(async () => {
  try {
    // @ts-ignore
    const r1csBuffer = await remix.call('fileManager', 'readFile', 'circuits/.bin/merkle_proof.r1cs', { encoding: null });
    // @ts-ignore
    const r1cs = new Uint8Array(r1csBuffer);
    // @ts-ignore
    await remix.call('circuit-compiler', 'compile', 'circuits/merkle_proof.circom');
    // @ts-ignore
    const wasmBuffer = await remix.call('fileManager', 'readFile', 'circuits/.bin/merkle_proof_js/merkle_proof.wasm', { encoding: null });
    // @ts-ignore
    ...
    // Build a simple depth-20 Merkle tree for demonstration
    const leafSecret = BigInt('42')
    const leaf = poseidon([leafSecret])

    // All siblings are the "zero hash" (Poseidon of zero) for an empty tree
    const zeroHash = poseidon([BigInt(0)])
    const pathElements = Array(20).fill(zeroHash.toString())
    const pathIndices = Array(20).fill('0')

    // Compute root: repeatedly hash (leaf, zeroHash) going up
